...
- [ ] 实现剪贴板监听器（轮询 / native API）
- [ ] 识别并区分剪贴板内容类型：文本 / 富文本 / 链接 / 图片 / 文件 / 颜色值
- [ ] 自动保存复制内容到本地数据库
- [ ] 去重逻辑：连续复制相同内容只保存一条
- [ ] 实现"暂停收集"开关（Pause Paste）
- [ ] 基础历史记录列表展示（时间倒序）
- [ ] 点击 item 直接粘贴到当前应用（Direct Paste）
- [ ] 支持纯文本粘贴（去除富文本格式）
- [ ] 全局快捷键呼出 / 收起主面板
...
- [ ] 按来源应用过滤
- [ ] 按日期范围过滤（今天 / 本周 / 自定义）
- [ ] 图片 OCR 文字识别并加入搜索索引（Vision framework）

---

## 👁️ Phase 4 · 预览与编辑

- [ ] 空格键快速预览 item（Quick Look 风格）
- [ ] 文本 item 内联编辑
- [ ] 链接 item 内置 WebView 预览
- [ ] 链接自动抓取元数据（标题 + 缩略图）
- [ ] 图片预览 + 旋转操作
- [ ] 颜色值预览 + 内置颜色选择器调整
- [ ] item 重命名
- [ ] item 右键上下文菜单（复制 / 固定 / 重命名 / 删除 / 分享）
- [ ] 多选 item（Shift / Cmd + 点击）执行批量操作
